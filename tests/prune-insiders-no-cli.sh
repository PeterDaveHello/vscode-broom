...
mkdir -p \
  ...
  "$HOME/.vscode-insiders/extensions/extA-0.9"
...
if [[ $STATUS -eq 0 ]]; then
  tap_ok "runs prune-uninstalled without desktop CLI"
else
  tap_not_ok "runs prune-uninstalled without desktop CLI"
  tap_diag "exit status: $STATUS"
fi

expect_match "prints prune skip message for missing code-insiders" "^Prune-uninstalled skipped for 1 scope \\(use -v for details\\)\\.$"
