...
mkdir -p \
  "$HOME/.vscode-insiders/extensions/extA-1.0" \
  "$HOME/.vscode-insiders/extensions/extB-1.0" \
  "$TMP_BIN"

cat << 'EOF' > "$TMP_BIN/code-insiders"
#!/usr/bin/env bash
set -euo pipefail
...
if [[ $STATUS -eq 0 ]]; then
  tap_ok "runs prune-uninstalled with code-insiders"
else
  tap_not_ok "runs prune-uninstalled with code-insiders"
...
expect_match "prints reclaimable path count is 1" "^Reclaimable \\(est\\.\\): .* \\(1 paths\\)$"
